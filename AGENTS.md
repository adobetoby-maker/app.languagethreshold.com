...
2. Read `ai-rosetta/README.md` and run `ai-rosetta/PREFLIGHT.md`.
3. Read the active task brief.
4. Update only `ai-rosetta/agents/CODEX_STATUS.md`.
5. During independent work, do not read Claude's plan until Codex's plan is marked `independent-complete`.
6. Work from a `codex/<task-slug>` branch, never directly from `main`.
7. Preserve dirty or divergent state; never reset or overwrite it to make a preflight pass.
8. Commit, push, fetch again, and verify the remote SHA before ending.
9. Production changes require Toby's explicit approval.
