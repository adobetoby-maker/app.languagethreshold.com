...
// ── Static catalog (built at module load, never network) ─────────────────────

/** Infinitive → language → table. Populated from FREQUENCY_CONJUGATIONS. */
const CATALOG: Store = {};
...
/** Register offline tables (e.g. profile-derived) into the static catalog. */
export function registerCatalogEntry(language: Language, set: ConjugationSet) {
  const key = norm(set.infinitive);
  if (!key) return;
  if (!CATALOG[key]) CATALOG[key] = {};
  // Prefer richer tables (ones with past+future) over sparse seeds
...
// ── User cache (AI results for novel verbs only) ─────────────────────────────
